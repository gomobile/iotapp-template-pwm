 ...
 * @member {Number} PWM class constructor argument, mraa PWM pin #
 * @member {Boolean} PWM class constructor argument, PWM object lifetime owner
 * @member {Number} PWM class constructor argument, PWM object "Chip ID"
 */
        ...
        switch( cfg.mraa.getPlatformType() ) {          // which board are we running on?
            ...
            case cfg.mraa.INTEL_NUC5:                   // Arduino 101 (aka "firmata") + 5th gen Broadwell NUCs
                ...
                if( /firmata$/.test(cfg.mraa.getPlatformName()) ) {
                    io = opt.altPin ? io : (3 + 512) ;  // use alternate pin?
                }
                else {
